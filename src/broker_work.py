...
4. Запланированное дело, в котором описано, что брокер сделает и почему
   (решение агентства от 28.08). Дело-заглушка «Позвонить» работой не
   становится, и засчитывается план только пока срок не наступил: как
   только срок прошёл, спрашивают уже за результат, а не за намерение.
...
# Запланированное дело, в котором описано, ЧТО брокер сделает и ПОЧЕМУ.
# Решение агентства от 28.08: «запланированное дело можно засчитать как
# работу, если в нём описаны шаги брокера и почему они такие». Дело-заглушка
# («Позвонить», «Связаться с клиентом») работой не становится: план, который
# нельзя проверить, ничем не отличается от его отсутствия. Признак считает
# код, а не модель, и он оспорим по буквам — см. _task_describes_the_plan.
...
# Дело в карточке есть, но по его тексту не понять ни что брокер сделает,
# ни почему. С 28.08 это уже не «в карточке только план»: описанное дело
...
# План в деле описан, но вернуться к клиенту брокер собрался позже нормы
# этапа. Отдельно от GAP_ONLY_PLANS: сказать «в деле не сказано, что и
# почему» про подробно расписанное дело — неправда, и брокер справедливо
...
RECORD_FIXES: dict[str, str] = {
    ...
    GAP_ONLY_PLANS: (
        "Дописать в деле, что именно сделаете и почему — по нынешней записи "
        "план не проверить"
    ),
}
...
REASON_RU: dict[str, str] = {
    ...
    PROVEN_BY_TASK_PLAN: (
        "в запланированном деле описано, что брокер сделает и почему"
    ),
    ...
    GAP_ONLY_PLANS: (
        "дело в карточке есть, но в нём не сказано, что брокер сделает и "
        "почему — ни звонка, ни комментария при этом нет"
    ),
    ...
    GAP_PLAN_TOO_FAR: (
        "план в деле описан, но вернуться к клиенту брокер собрался позже "
        "нормы этапа"
    ),
...
# Сколько осмысленного текста должно быть в деле, чтобы считать его планом.
# Число выведено из уже принятого агентством порога: BUYER_OFER_MIN_COMMENT_LEN
# = 30 символов — это планка «развёрнутого комментария», отвечающего на ОДИН
# вопрос (что происходит). Здесь вопроса два — что брокер сделает и почему, —
# поэтому планка удвоена. Цифру придётся защищать перед брокером, у которого
# дело на 55 символов не засчитали, и защищается она именно этим счётом.
TASK_PLAN_MIN_LEN = 60
...
def _task_plan_state(
    ...
    Решение агентства от 28.08. Признак считает код: «описано» — это текст
    брокера длиной от TASK_PLAN_MIN_LEN, в котором есть что-то помимо
    шаблонной темы. Спросить об этом модель было бы проще и хуже: суждение о
    работе человека должно быть оспоримым по буквам, а не по впечатлению.

    Дело должно быть живым — незакрытым и со сроком в будущем. Закрытое дело
    — уже не план, а дело без срока (Битрикс ставит 9999-12-31) не держит
    ничего: засчитав его, мы вернули бы «Дело стоит на 9999-12-31 — ждём».

    И срок должен укладываться в норму этапа. Без этого правило вырождается
    ровно так же, как выродилась бы пауза без _pause_covers_the_task:
    достаточно один раз подробно расписать дело на два года вперёд — и работа
    за каждое окно засчитана навсегда. Норма этапа — это и есть срок, за
    который агентство ждёт возврата к клиенту; планировать дальше можно, но
    тогда надо написать почему, и это уже объяснённая пауза, а не план.
    """
    ...
    for event in events:
        ...
        if not words or all(_is_template_word(word) for word in words):
            continue
        if deadline > horizon:
...
def due_task_without_result(
    ...
    for event in events:
        ...
        if _day_start(deadline) > today:
            continue
        candidates.append((deadline, str(event.get("subject") or "").strip()))
    if not candidates:
        return None
    ...
    for due, subject in sorted(candidates):
...
def next_action(
    ...
    if due is not None:
        ...
        return (
            f"Срок дела сегодня ({due['deadline']}) уже прошёл — "
