...
def due_task_without_result(
    ...
    for due, subject, event in sorted(candidates, key=lambda c: c[0]):
        # Запись брокера может лежать в самом деле — решение агентства от
        # 31.08. Напоминать «выполни и отпишись» по делу, в названии
        # которого отписка и записана, значит просить сделать сделанное.
        if _task_has_the_brokers_words(event):
            continue
