...
#
# Место — это ещё и общая мера. Внутри места советы соревнуются по weight,
# и сравнивать их можно, только пока вес у всех в одних единицах: рубли с
# рублями, карточки с карточками. Просроченные обещания стояли рядом с
# событиями по сделкам, где вес — сумма на карточке, и проигрывали ей
# всегда: двадцать девять обещаний это вес 2929, а одна ушедшая сделка на
# 2,6 млн — вес 2 600 000. Самый сильный сигнал витрины не мог быть
# высказан ни разу, и заметить это по коду было нельзя — только по тому,
...
def select(
    ...
    *,
    ...
    ХВАЛИТЬ МОЖЕТ ТОЛЬКО ТОТ, КТО ГОВОРИЛ, — для этого praise=False.

    «Сработало» строится на сравнении памяти с сегодняшними кандидатами, и
    верно это ровно тогда, когда кандидаты считаются ТАК ЖЕ, как в тот раз,
    когда совет прозвучал. Память пишет рассылка, и пишет по всей компании.
    Экран РОПа считает на суженном соединении: совета про чужой отдел он
    увидеть не может — и принимал это за «проблема решена, стало ноль».

    Мало того что неправда: имя для похвалы берётся из памяти, когда
    кандидата нет, — то есть РОП читал «Отдел Кретов: было 35 400 250,
    стало 0». Чужой отдел, чужие деньги, и всё это выдумано.

    Память при этом остаётся нужна и без похвалы: по ней держится пауза на
    повтор. Выключается именно суждение «сработало», а не чтение памяти.
    """
    moment = now or _now()
    current = {advice.key: advice for advice in candidates}
    result = Selection(
        resolved=_resolved(current, memory, moment) if praise else [],
    )
