...
# не ищет. Задача, которой он не нужен, зовёт quiet_the_portal_client();
# по умолчанию поведение прежнее.
BX_VERBOSE = True


def quiet_the_portal_client() -> None:
    """Убрать из лога построчный отчёт клиента портала.

    fast_bitrix24 пишет INFO на каждый запрос и рисует прогрессбар. На
    ручном прогоне это полезно, в cron — нет: полная выгрузка досье делает
    две с половиной тысячи запросов, догрузка расшифровок — до четырёхсот,
    и столько же пар строк уходит в общий cron.log, где их никто не ищет.
    Однажды этот файл уже дорос до 14 ГБ при диске в 41 (см.
    scripts/cron_job.sh), и класть туда мегабайт в день незачем.

    Живёт рядом с флагом, а не в задаче: выключателей у одной лампы должно
    быть столько же, сколько ламп. Пока он был приватным в dossier, вторая
    задача с тем же прогрессбаром завела бы себе вторую копию — и они
    разошлись бы в тот день, когда одну поправят.

    Гасится только шум. Ошибки портала остаются: уровень WARNING, а не
    CRITICAL. Итоги прогона задача пишет сама — одной строкой.
    """
    global BX_VERBOSE
