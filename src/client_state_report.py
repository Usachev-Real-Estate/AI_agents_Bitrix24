...
def split_sections(
    ...
    for result in results:
        ...
        is_neglected = (
            ...
            and reason not in WORK_REMINDERS and not is_abandoned
        )
        if is_losing:
...
def format_sections(
    ...
    parts = []
    if shared:
        parts.append(f"{shared} {abandoned_noun(shared)}")
    if not_started:
        parts.append(f"{len(not_started)} без начатой работы")
    if parts:
        tail = "списками" if len(parts) > 1 else "списком"
        overlap = f"Из них {' и '.join(parts)} — отдельными {tail} ниже."
        if len(parts) == 1:
            overlap = f"Из них {parts[0]} — отдельным {tail} ниже."
    ...
    if not_started:
        # Свой список — по той же причине, что у брошенных: в общем перечне
        # он теряется, а разговор с брокером тут другой. Не «верните
        # клиента», а «начните работать». Решение агентства от 31.08.
        _block(
