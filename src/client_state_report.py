...
# отдела, и полный разбор остаётся в JSON прогона. Остаток называется
# числом по разделам — иначе это снова было бы отсутствие, выданное за
# результат.
#
# Потолок делится МЕЖДУ разделами, а не тратится сверху вниз одним котлом.
# Прогон 01.09 14:30 показал, чем кончается общий котёл: у Кретова 358
# карточек, из них 247 с вопросами, разделы печатались по строгости — и
# 🆕 съедал остаток целиком. В сообщение уходило 39 разборов, из которых
# в 🔧 НЕДОРАБОТКА — ни одного: все 72 уходили в строку остатка числом.
# Отчёт, заведённый чтобы РОП контролировал работу брокеров, раздел про
# работу брокеров не показывал.
#
# Доля считается от ОСТАТКА потолка и числа ещё не напечатанных разделов
# (_section_share): раздел, которому нужно меньше доли, отдаёт лишнее
# следующим, а последний забирает всё, что осталось. Так ни один раздел не
# остаётся без голоса и ни один не пропадает даром.
...
# заголовка, и молча.
# 🆕 здесь нет намеренно: с 01.09 раздел печатается списком целиком и в
# остаток не попадает никогда.
...
def _section_share(pot: int, sections_left: int) -> int:
    """Сколько полных разборов достаётся разделу из оставшегося потолка.

    sections_left считает и текущий раздел тоже. Последнему достаётся весь
    остаток: делить уже не с кем, и придержать место было бы не бережностью,
    а потерянной строкой.

    Не меньше одного, пока в потолке хоть что-то есть. Раздел, у которого
    ноль разборов, читается как пустой — а он не пуст, и число в заголовке
    это прямо говорит. Голос важнее пропорции: РОП должен увидеть, КАК
    выглядит претензия в каждом разделе, а не только сколько их.
    """
    if sections_left <= 1:
        return pot
    return max(1, pot // sections_left)
...
def format_sections(
    ...
    # Разделы, которые делят потолок разборов, в порядке печати. Список
    # строится по тем же условиям, по которым разделы печатаются ниже:
    # доля считается от числа ЕЩЁ НЕ напечатанных разделов, и раздел,
    # которого в отчёте не будет, долю занимать не должен.
    #
    # 🆕 сюда не входит: он печатается списком и разборов не тратит.
    queue = ["losing"]
    if abandoned:
        queue.append("abandoned")
    queue.append("neglected")
    if reminders:
        queue.append("reminders")

    def _block(
        header: str, rows: list[dict[str, Any]], empty: str, key: str = "",
    ) -> None:
        # Раздел выбывает из очереди сразу, до всех ранних выходов. Иначе
        # свёрнутый или пустой раздел, не потративший ни одного разбора,
        # продолжал бы считаться делителем — и урезал бы долю тем, кто идёт
        # ниже, ради места, которое сам не занял.
        if key in queue:
            queue.remove(key)
        ...
        if not rows:
            ...
            blocks.append("")
            return
        # Своя доля потолка, а не весь остаток: раздел, идущий первым, не
        # должен съедать место у тех, что ниже. Неизрасходованное остаётся
        # в потолке и достаётся следующим разделам.
        quota = _section_share(budget[0], len(queue) + 1)
        left = 0
        for row in rows:
            deal_id = int(row.get("deal_id") or 0)
            title = titles.get(deal_id, "")
            if deal_id not in printed and (quota <= 0 or budget[0] <= 0):
                # Доля выбрана. Карточку не печатаем, но и не теряем: её
    ...
    if not_started:
        # Свой раздел — с 31.08, и списком — с 01.09. Полный разбор тут
        # нечего читать: работы не было, и девять строк про температуру,
        # ситуацию и совет говорят об одном — «начните работать». Прогон
        # 01.09 14:30 показал цену: 93 таких карточки у одного РОПа съедали
        # весь потолок разборов, и 🔧 НЕДОРАБОТКА не получал ни одного.
        #
        # Списком раздел печатается ЦЕЛИКОМ, без потолка: это перепись, и
        # обрезанная перепись бесполезна — РОП не сможет раздать карточки
        # брокерам, если половины в списке нет. Строка на карточку стоит
        # примерно в девять раз дешевле разбора.
        blocks.append(f"[B]🆕 РАБОТУ НЕ НАЧИНАЛИ — {len(not_started)}[/B]")
        for row in not_started:
            deal_id = int(row.get("deal_id") or 0)
            printed.add(deal_id)
            name = card_title(titles.get(deal_id, ""))
            blocks.append(
                f"#{deal_id} {name} — [URL]{deal_url(webhook_url, deal_id)}"
                f"[/URL]".strip(),
            )
        blocks.append("")
