    ...
    if work and not work.get("proven"):
        ...
        if reason_code == GAP_ONLY_PLANS and work.get("task_text"):
            ...
            if not isinstance(quiet, (int, float)):
                ...
                # как ошибка отчёта.
                tail = (
                    "следов брокера нет вовсе, "
                    f"карточка на этапе {float(quiet):.0f} дн."
                )
            else:
                tail = f"ни звонка, ни комментария брокера {float(quiet):.0f} дн."
...
#
# Отсюда ровно три кода, и каждый проверяется по буквам определения:
...
LOSING_GAPS = frozenset({GAP_NO_TRACE, GAP_NO_TRACE_IN_WINDOW, GAP_ABANDONED})
