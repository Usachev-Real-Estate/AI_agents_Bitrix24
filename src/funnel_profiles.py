...
10. client_refused=true, если контрагент ПРЯМО отказался от сделки как
   таковой: не продаёт и не собирается, не покупает, передумал, снял
   объект, работает с другим агентством, «не звоните больше». Обязательно
   приведи цитату в client_refused_quote — ровно ту фразу, где отказ
   назван. Кто её записал, неважно: отказ — это состояние клиента.
   Это НЕ отказ: не подошёл конкретный объект, не устроила цена, «дорого»,
   «подумаем», «пока не готов», названная пауза, молчание клиента. Там
   работа продолжается, а тут её предмет исчез.
