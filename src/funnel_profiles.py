...
9. broker_work — про то, ЧЕМ брокер подтверждает работу, а не хорошо ли он
   ...
   ВАЖНО, ЧЬИ ЗАПИСИ ЧИТАТЬ. У каждого события есть поле by_responsible.
   true — это запись ответственного брокера или его РОПа, false — чужая
   (бэк-офис, робот, бизнес-процесс). Три флага ниже — claims_messaged,
   comment_informative, claims_no_answer — это утверждения О РАБОТЕ БРОКЕРА,
   и ставить их можно ТОЛЬКО по событиям с by_responsible=true. Чужой
   комментарий читай, чтобы понять клиента, но за работу брокера не
   засчитывай и цитаты для этих трёх флагов из него не бери.
   Исключение одно — pause_explained: причина паузы верна независимо от
   того, кто её записал, и её ищи во всех комментариях и расшифровках.

   * claims_messaged=true, если в последних комментариях ОТВЕТСТВЕННОГО
     брокер утверждает, что писал клиенту (в вотсап, телеграм, почту,
     «отправил», «скинул»). Обязательно приведи цитату в
     claims_messaged_quote. Не нашёл — false.
   * comment_informative=true, если по последним комментариям ОТВЕТСТВЕННОГО
     понятно, что происходит с клиентом и что дальше. «Созвонился», «ок»,
     «ждём» — это false: такой комментарий не заменяет разговора.
     Содержательный комментарий чужого автора флаг НЕ ставит: вопрос в том,
     что записал брокер, а не в том, что можно узнать из карточки.
   * claims_no_answer=true ТОЛЬКО если в комментарии ОТВЕТСТВЕННОГО брокер
     объясняет отсутствие продвижения
   ...
   * pause_explained=true, если в ЛЮБЫХ комментариях или расшифровке НАЗВАНА
     ПРИЧИНА паузы на стороне клиента: отпуск, командировка, лечение, разъезды, ждёт
...
10. client_refused=true в двух случаях. Первый — контрагент ПРЯМО
   отказался от сделки как таковой: не продаёт и не собирается, не
   покупает, передумал, снял объект, «не звоните больше».
   Второй — он отказался работать С НАМИ: не платит комиссию, не
   подписывает договор, не обсуждает условия сотрудничества, ушёл к
   другому агентству. Сделка при этом может быть жива, но нашей она уже
   не будет, и держать её в воронке не за что.
   Обязательно приведи цитату в client_refused_quote — ровно ту фразу, где
   отказ назван. Кто её записал, неважно: отказ — это состояние клиента.
   Это НЕ отказ: не подошёл конкретный объект, не устроила цена объекта,
   «дорого», «подумаем», «пока не готов», названная пауза, молчание
   клиента, торг об условиях. Там работа продолжается, а тут её предмет
   исчез. Разница между «завышенная цена» и «отказом платить комиссию»
   именно в этом: первое — предмет переговоров, второе — их конец.
   Флаг снимает с карточки все требования к темпу и ведёт к одному
   действию — перенести сделку в проигранные. Сомневаешься — false.
"""
...
SELLER_STAGE_REQUIREMENTS: dict[str, tuple[tuple[str, str], ...]] = {
    ...
    # стоял «у руководства». Набор фактов взят у «Подготовки объекта в
    # рекламу»: тот этап из аудита уходит, и «Переговоры» становятся поздним
    # этапом воронки, где цена выставления уже должна быть названа.
    "UC_KEOOG8": (
        ("property_address", "адрес объекта"),
        ("property_type", "тип объекта"),
        ("selling_timeline", "срок продажи"),
