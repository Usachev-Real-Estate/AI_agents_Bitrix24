...
# Чем в карточке записывают отказ клиента (правило 2 раздела 6.2 ТЗ).
#
# ФРАЗАМИ, а не отдельными словами, и это не вкусовщина. Правило отказа
# стоит вторым сверху, то есть перебивает и тишину, и «движется»: ложное
# срабатывание убирает живого клиента из списка брокера, и убирает молча.
# Одно слово «отказ» поймало бы и «клиент не отказывается», и «отказ по
# мощностям» в служебной записи. Фраза длиннее и потому промахивается реже.
#
# Слова «дорого» и «отложил» здесь нет нарочно: первое брокер пишет
# посреди торга, второе — это отдельная стадия «Отложенная продажа», а не
# конец работы.
#
# Список заведомо неполон — его правит агентство под свои формулировки,
# для этого он и вынесен в настройку. Пустой список гасит правило 2
# целиком, и прогон обязан сказать об этом в degraded_rules (раздел 10).
REFUSAL_MARKERS: tuple[str, ...] = (
    ...
    "ипотеку не одобрили",
    "банк отказал",
    "работает с другим агент",
    "ушел к другому",
    "ушел в другое агентство",
    "расторгли договор",
...
class Settings(BaseSettings):
    ...
    #
    # Токены машинного доступа к книге клиентов (раздел 7.1 ТЗ). Пустой
    # токен — ветка выключена: пустая строка не должна впускать запрос с
    # пустым `Authorization: Bearer`.
    #
    # Приставка DOSSIER_ взята из ТЗ, хотя читают токены не досье, а ручки
    # дашборда под `{base_path}/api`. Расходиться с ТЗ в именах настроек
    # дважды (после REFUSAL_MARKERS, [V26]) значило бы превратить его в
    # текст, по которому нельзя настроить продукт.
    #
    # **Токен — доступ ко ВСЕМУ портфелю**, без сужения по отделам. Это
    # осознанно: наружу читает модель, а не человек. Отсюда и запрет на
    # выдачу его брокеру: область видимости у него не сузится.
    dossier_read_token: str = Field(
        default="",
        validation_alias="DOSSIER_READ_TOKEN",
    )
    # Тот же доступ плюс право дописать разбор. Отдельным токеном, а не
    # флагом у первого: читающих может быть много, пишущий один, и
    # отобрать право записи иначе можно только сменив ключ всем.
    dossier_write_token: str = Field(
        default="",
        validation_alias="DOSSIER_WRITE_TOKEN",
    )
    ...
    def refusal_markers(self) -> tuple[str, ...]:
        """Маркеры отказа. Пустой кортеж — правило 2 гаснет и уходит в degraded.

        Имя без приставки DOSSIER_, которой настройку зовёт ТЗ `[V26]`:
        досье к ней отношения не имеет и никогда не имело — там отказ
        определяет модель, а не слова. Приставка обещала бы читателю
        связь, которой нет, и увела бы его чинить не тот модуль.
        """
        try:
            values = json.loads(self.refusal_markers_json)
        except Exception:
            return REFUSAL_MARKERS
        # Разобрался — ещё не значит «список». `{"а": 1}` переберётся по
        # ключам, а голая строка `"передумал"` — по буквам, и маркером
        # станет каждая; буква совпадает почти с любым текстом.
        if not isinstance(values, list):
            return REFUSAL_MARKERS
...
# Клиенты HTTP, которых незачем слушать построчно.
#
# httpx пишет на уровне INFO строку на каждый запрос — вместе с ПОЛНЫМ
# адресом, а в адресе вебхука Битрикса лежит токен. Полтораста запросов на
# тик, тик раз в пятнадцать минут — это порядка пятнадцати тысяч копий
# токена в сутки в logs/cron.log: в файле, который лежит рядом с кодом,
# уезжает в любую пересылку лога и однажды уже дорастал до 14 ГБ.
# На уровне DEBUG httpx и httpcore дописывают туда же заголовки.
#
# Гасится только шум. WARNING и выше проходят, то есть отказ портала
# по-прежнему виден, а счётчик своих запросов каждая задача пишет сама.
_NOISY_HTTP_LOGGERS = ("httpx", "httpcore")


def quiet_http_clients() -> None:
    """Убрать из лога построчный отчёт HTTP-клиентов вместе с токеном.

    Зовётся из `setup_logging`, то есть срабатывает сама у всех задач,
    которые её используют. Точкам входа, настраивающим логи иначе — через
    logging.basicConfig, — приходится звать её отдельно, и за тем, чтобы
    ни одна не забыла, следит отдельный тест.

    (Имя выше нарочно без скобок. Соседний тест грепает каталог на вызов
    настройки логов с пустыми скобками — без уровня журнала, — и
    упоминание в прозе он не отличает от вызова. Ловит он настоящую
    ошибку, так что проще его не дразнить.)
    """
...
def setup_logging(level: str) -> None:
    ...
    # Последним — и намеренно ПОСЛЕ установки уровня корня: при
    # LOG_LEVEL=DEBUG корень опускается до DEBUG, и без этой строки httpx
    # вывалил бы в лог не только адрес с токеном, но и заголовки.
    quiet_http_clients()
