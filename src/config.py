...
class Settings(BaseSettings):
    ...
    # Цена перечитывания закэшированного входа. Отмасштабирована в той же
    # пропорции к входу, что стояла раньше (4.04 к 40), — ПРОВЕРИТЬ НЕЧЕМ:
    # за 01.09 провайдер вернул 0 закэшированных токенов на все 758 вызовов,
    # то есть постоянная часть запроса каждый раз оплачивалась заново. Как
    # только кэш заработает, ставку надо свести с выгрузкой так же, как
    # сведены две верхние.
    llm_price_cache_read: float = Field(
        default=8.5062, validation_alias="LLM_PRICE_CACHE_READ",
    )
