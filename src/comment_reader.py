...
#
# 5: широкий срок — это его ПОСЛЕДНИЙ день («осенью» → 30.11, а не
#    01.09), и обещание не достраивается за риелтора, если следующий
#    шаг напрашивается сам.
...
15 августа того года, в котором запись. Год бери из даты записи; если срок
явно перешёл на следующий год, увеличь год.

ШИРОКИЙ СРОК — ЭТО ЕГО ПОСЛЕДНИЙ ДЕНЬ, а не первый. «Осенью» — 30 ноября.
«В сентябре» и «с сентября» — 30 сентября. «На следующей неделе» — её
пятница. «Провести показы 09.09–15.09» — 15 сентября. Обещание не
нарушено, пока не прошёл последний возможный день: взяв первый, мы
превращаем план в обвинение и упрекаем человека, который ничего не
нарушил.
...
НЕ ДОСТРАИВАЙ ЗА РИЕЛТОРА. Рассказ о сделанном не становится обещанием
оттого, что следующий шаг напрашивается сам. «Скинул планировку, квартира
не подошла, отправил другие варианты» — здесь нет ни обещания, ни срока,
как бы очевидно ни было, что дальше надо позвонить. Придуманное обещание
опаснее пропущенного: через три дня оно станет просрочкой, и человеку
предъявят слова, которых он не говорил.

Срок берётся только из текста. Дня в записи нет — promised_at остаётся
null, даже когда обещание есть.
...
Запись от 2026-08-10: «Связаться осенью»
{"promised": "Связаться", "promised_at": "2026-11-30", "wait_until": null,
 "refused": false, "refused_why": "", "ready": "", "terms": ""}

Запись от 2026-09-10: «Скинул планировку, квартира не подошла. Отправил
еще другие варианты которые у нас есть»
{"promised": "", "promised_at": null, "wait_until": null, "refused": false,
 "refused_why": "", "ready": "отправлены другие варианты", "terms": ""}
...
def read_cards(conn, llm, *, limit: int = BATCH, today: date | None = None,
    ...
    try:
        ...
        for future in as_completed(pending):
            ...
            #
            # Соседи. Транзакция открыта на весь сеанс, и полный проход
            # держал блокировку записи тридцать две минуты подряд — ETL
            # тикнул в середине, подождал свои десять секунд и упал с
            # «database is locked». Коротким коммитом блокировка живёт
            # мгновение вместо получаса.
            #
            # Свои деньги. Обрыв на тысячной карточке откатывал ВСЮ
