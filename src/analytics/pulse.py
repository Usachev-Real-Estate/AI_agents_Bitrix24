...
Главное правило этого файла: **факт считается по тому же множеству людей, из
которого сложен план.** Иначе выполнение систематически завышено. План
третьего квартала несут 29 названных человек; в тех же отделах работают ещё
двадцать — новички без нормы и руководители, — и их сделки это настоящие
деньги компании, но не выполнение чьей-то цели. Поэтому «прочие» стоят
отдельной строкой с собственной суммой, а итог компании — сумма обеих строк.
Так число сходится по строкам, и разрыв назван, а не спрятан.
...
def pulse(
    ...
    departments, total_fact, others_fact, others_deals = [], 0.0, 0.0, 0
    for row in plan["departments"]:
        own = [f for f in facts if f["department_id"] == row["department_id"]]
        fact = sum(f["amount"] for f in own if f["user_id"] in on_plan_ids)
        rest = sum(f["amount"] for f in own if f["user_id"] not in on_plan_ids)
        total_fact += fact
        others_fact += rest
        ...
        departments.append({
            ...
            "deals": sum(f["deals"] for f in own if f["user_id"] in on_plan_ids),
            "on_plan": row["on_plan"],
    ...
    return {
        ...
        # Деньги тех, кто плана не несёт. Не «лишние» и не «ошибка»: это
        # выручка новичков и руководителей. Без этой строки итог компании не
        # сходится с суммой сделок, и первый же вопрос про расхождение
        # обрушит доверие ко всему экрану.
        "others": {
            "fact": round(others_fact, 2),
            "deals": others_deals,
