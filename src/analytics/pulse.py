...
def pulse(
    ...
    return {
        ...
        # считается по «Покупателям», а на соседних страницах числа собраны
        # по другой воронке, и два разных факта без подписи читаются как
        ...
        #
        # И только собственнику. Порог считается из расходов ВСЕЙ компании,
        # а факт на суженном соединении — только по отделам РОПа: их
        # разность не значит ничего, «не хватает 22 млн» при том, что
        # компания у самого рубежа. Заодно из такой строки читается размер
        # расходов агентства — число, которое собственник РОПу не называл.
        #
        # Закрыто здесь, а не в правиле советов: тогда рубеж исчезает и с
        # «Пульса», и из «Плана на день», и из совета одним решением, и
        # новый экран не сможет его показать по забывчивости.
        "breakeven": (
            plans.breakeven(
                total_fact, projection, period["starts_at"], period["ends_at"],
            )
            if scope_of.is_unrestricted(conn) else None
        ),
