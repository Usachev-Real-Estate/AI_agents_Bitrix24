...
# «Разбор ещё не устарел»: после чтения человеческих записей не появилось.
#
# Решение агентства от 15.09: брокер вернулся к карточке — обещание
# закрыто. Написал результат, «не дозвонился», «перенесли» — что угодно;
# спрашивать за молчание, которого не было, значит предъявить упрёк на
# пустом месте, а один такой упрёк обесценивает соседние верные строки.
#
# Раньше это держалось на перечтении: новая запись меняет отпечаток,
# карточку читают заново, обещание там уже другое. Но перечтение отстаёт —
# читатель берёт 400 карточек за ночь из тысячи с лишним, и очередь идёт
# по кругу. Брокер закрывал обещание сегодня, а дашборд упрекал его этим
# ещё двое-трое суток.
#
# Витрина отвечает сама и сразу: комментарии обновляет ETL каждые
# пятнадцать минут. Появилась запись после чтения — разбор устарел, что в
# ней написано, неизвестно, и обвинять не на чем. Перечтение потом скажет
# своё слово; до тех пор карточка молчит. Ошибка здесь в сторону молчания,
# и это правильная сторона.
#
# Записи робота не в счёт: «Новое обращение: звонок с Cian» — не
# возвращение человека к карточке.
#
# julianday, а не сравнение строк: время записи приходит из портала со
# смещением +03:00, время чтения пишется в UTC, и лексикографически
# «10:00:00+03:00» больше «08:30:00+00:00», хотя случилось раньше.
_NOT_STALE = """
          AND NOT EXISTS (
                SELECT 1 FROM v_comment c
                 WHERE c.entity_type = 'deal' AND c.entity_id = d.deal_id
                   AND c.is_auto = 0
                   AND julianday(c.created_at) > julianday(r.read_at))
"""
...
def promises(
    ...
    *,
    ...
    Обещание, после которого брокер вернулся к карточке, снимается: он
    написал результат, «не дозвонился», «перенесли» — что угодно, и
    спрашивать с него за молчание, которого не было, не за что.

    Раньше это держалось на перечтении, и перечтение отставало на двое-трое
    суток. Теперь витрина отвечает сама — см. _NOT_STALE.
    """
    where, params = plans.category_filter("d", categories)
...
def refused_in_work(
    ...
    *,
    ...
    поле пустое. Но перечтение отстаёт, поэтому устаревший разбор снимается
    и здесь — тем же _NOT_STALE, что и обещание: вопрос один, ответ обязан
    быть один.
    """
    where, params = plans.category_filter("d", categories)
    params["dept"] = department_id
