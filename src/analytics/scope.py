...
отделов, а запросы метрик обращаются только к ним. Тем же приёмом убраны с
дашборда уволенные: v_user — состав, о котором идёт разговор, v_user_all —
справочник для привязки карточки к человеку. Новый запрос получает
...
_VIEW_DDL = (
    ...
    #
    # Уволенных здесь нет. Представлений о людях два, и разница между ними
    # — это разница между «о ком дашборд говорит» и «чья это карточка».
    #
    # v_user — состав, о котором идёт разговор: рейтинги, списки, выпадающие
    # списки отделов, «кто не берёт трубку», «кто молчит». Уволенному там
    # нечего делать: строку про него нельзя ни выполнить, ни спросить, а
    # место в рейтинге она занимает.
    #
    # v_user_all — справочник для привязки: чья карточка, какого отдела
    # сделка, чьи деньги. Уволенный сотрудник ушёл, а его карточки и
    # закрытые им сделки остались, и вычесть их значит напечатать неверную
    # сумму по отделу и потерять карточки, которые как раз некому вести.
    #
    # Короткое имя досталось суженному представлению нарочно: запрос,
    # написанный не глядя, покажет живых. Полный справочник приходится
    # назвать вслух — и это ровно то место, где стоит подумать.
    """
    CREATE TEMP VIEW v_user_all AS
    ...
    SELECT u.* FROM v_user_all u WHERE u.is_active = 1
    """,
...
def is_unrestricted(conn) -> bool:
    """Видит ли это соединение всю компанию.

    Нужно там, где число приходит НЕ из витрины и потому не сужается
    представлением. Пример ровно один и показателен: рубеж безубыточности
    считается из расходов агентства, заданных настройкой, а сравнивается с
    фактом — а факт на суженном соединении только по отделам РОПа. Их
    разность не значит ничего и заодно называет размер расходов компании.

    Спрашивается сам признак, а не пустота списка отделов: «видеть всё» —
    это роль администратора, а не отсутствие ограничений. Соединение без
    области видимости считается ограниченным, а не открытым: конструкция
    закрывается при сбое, а не открывается.
    """
    try:
        row = conn.execute("SELECT unrestricted FROM scope_flag").fetchone()
    except Exception:  # соединение без области видимости — значит ничего
        return False
