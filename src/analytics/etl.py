...
def sync_deals(
    ...
    *,
    ...
    if batch:
        conn.executemany(_DEAL_UPSERT, batch)
    # Хвостовой сброс тоже отпускаем, и это не симметрия ради
    # красоты. При боевом HISTORY_BATCH=500 обычный прогон в
    # ветку внутри цикла не заходит НИ РАЗУ: единственная запись
    # — вот эта, и открытую ею транзакцию держал бы уже
    # следующий загрузчик, пока листает портал.
    release(conn)
...
def release(conn, stage: str = "") -> None:
    ...
    И следствие, без которого правило не работает: загрузчик не возвращает
    управление с открытой транзакцией. Полагаться на то, что за ним идёт
    не сеть, нельзя — порядок вызовов в run_sync меняют, а замок при этом
    уезжает в чужую функцию молча. Первая редакция этой правки на том и
    попалась: sync_deals отдавал управление с открытым хвостовым сбросом,
    и его транзакцию держали всю постраничную выгрузку лидов.
...
def run_sync(kind: str, *, since_override: str | None = None) -> dict[str, Any]:
    ...
    with BitrixClient(
        ...
        with etl_run(conn, kind) as counters:
            ...
            # Знак дел ставится ПОСЛЕ самих дел, и важен именно порядок, а
            # не общий коммит: с тех пор как загрузчик отпускает и хвостовой
            # сброс, данные уезжают раньше знака. Это безопасная сторона —
            # прогон, упавший между ними, оставит дела без знака, и следующий
            # заберёт то же окно заново, upsert'ом. Обратный порядок — знак
            # без данных — терял бы их насовсем.
