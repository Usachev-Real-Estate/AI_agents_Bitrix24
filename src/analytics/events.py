...
def _moves(
    ...
    return _rows(
        ...
        JOIN v_stage_event e2
         ...
         AND e2.seq = e1.seq + 1
         -- Только внутри одной воронки. Карточку берут из общей базы в
         -- рабочую воронку, и на экране это читалось как «вернулись
         -- назад»: «Первый показ» → «C26:NEW», 300 млн, красным.
         --
         -- «Назад» определяется сравнением порядковых номеров стадий, а
         -- номера у каждой воронки свои. «Первый показ» в одной и «NEW» в
         -- другой — числа из двух не связанных рядов, и разность их не
         -- значит ничего; вдобавок название чужой стадии не находится в
         -- справочнике своей воронки, sort падает в ноль, и знак разности
         -- определяется тем, какая из двух не нашлась.
         --
         -- Перевод в другую воронку — отдельное событие, и мерить его
         -- шкалой одной воронки нельзя. Врать тут дороже, чем промолчать:
         -- РОП получает красную строку про сделку, с которой всё в
         -- порядке, и следующую красную читает уже недоверчиво.
