...
# бюджета. Положить их в deferred значит выдать «данных недостаточно» трети
# всех карточек портфеля (30 % звонков длятся ровно ноль секунд), и правило
# README, ради которого затевался queued, перестанет что-либо значить.
T_TOO_SHORT = "too_short"

# Ниже этого порога разговора не было. Замер на боевом портале: 30 % звонков
# длятся 0 секунд, ещё 31 % — меньше тридцати. Отсечка снимает около 78 %
# обращений за расшифровками ДО первого запроса, и стоит она ноль: обе
# границы приходят вместе со списком дел.
LONG_CALL_SEC = 60
