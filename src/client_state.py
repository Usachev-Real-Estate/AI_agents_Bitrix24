...
#
# Подряд считается по ОТВЕТАМ МОДЕЛИ, а не по карточкам. Карточка из кэша
# модель не звала и о её здоровье не говорит ничего; сбрасывать на ней
# счётчик значило бы считать молчание ответом — ровно та подмена, с которой
# этот проект борется везде. В портфеле, где из кэша приходит девять
# карточек из десяти, серия из пяти подряд собиралась бы только по
# случайности порядка: провайдер лежит, полсотни карточек не прочитаны, а
# между ними кэш обнуляет счёт — и отчёт уходит РОПу как настоящий.
LLM_FAILURE_STREAK = 5

# Ответ модели получен: разбор удался (reason пуст) либо пришёл, но не
# разобрался. Оба случая доказывают, что модель на связи, и оба сбрасывают
# серию. Всё остальное — карточки, которые до модели не дошли: кэш,
# пропущенный этап, несобранная карточка. Они счёт не трогают.
LLM_ANSWERED = frozenset({"", "parse_error"})
...
def run_client_state(
    ...
    *,
    ...
    for deal in deals:
        ...
        card_reason = str(result.get("reason") or "")
        if card_reason == "llm_error":
            ...
            if consecutive_llm_errors >= LLM_FAILURE_STREAK:
                ...
                break
        elif card_reason in LLM_ANSWERED:
            # Модель ответила — серия прервана. А карточка, которая до
            # модели не дошла (кэш, пропущенный этап), счёт не трогает: она
            # не свидетель ни за, ни против.
            consecutive_llm_errors = 0
