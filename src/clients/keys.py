...
# перед нами один человек. См. split_shared.
#
# Четыре — это единица сверх измеренного. Замер 24.09 на портфеле в 2586
# контактов: имён, носимых несколькими группами, девять, и разброс у них
# `{2: 7, 3: 2}` — семь имён на двух группах, два на трёх. Больше трёх нет
# ни одного. Так выглядят тёзки: имя распространённое, а карточек с ним в
# портфеле три-четыре. Заполнитель выглядел бы иначе — одна подпись на
# десятке групп и больше, потому что её получает каждая карточка, заведённая
# автоматом.
#
# Первая редакция порога стояла на двойке и отказывала всем девяти, то есть
# двадцати номерам из ста девятнадцати кандидатов. Отказ был написан против
# заполнителя, а попал по тёзкам: заполнителя в портфеле не оказалось вовсе.
#
# Порог держится на замере, а не на догадке, и замер продолжает считаться
# каждым прогоном (`namesake_spread` в разборе). Сдвинулось распределение —
# видно сразу, до того как это что-то стоило.
NAMESAKE_LIMIT = 4
