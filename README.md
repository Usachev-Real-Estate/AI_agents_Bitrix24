...
**Снятия с рекламы временно выключены на странице.** Ни чипа «Сняты с
рекламы», ни вида «Снятия за период», ни плитки «Снято с рекламы за период»
сейчас нет: раздел о них не спрашивают. Выключение сделано тремя
закомментированными строками в `objects.py` (`VIEWS`, `LISTING_FILTERS`,
`SECOND_METRICS`) — весь остальной код снятий цел и просто не вызывается.
Вернуть: снять эти три комментария и убрать маркер `removals_off` в
`tests/test_dashboard_objects.py`, после чего вернутся и одиннадцать
пропущенных проверок. Ниже описано поведение, которое включится вместе с ними.

Под фильтром «в рекламе» плиток за период две: выставлено и снято. Раньше на
месте второй стояло состояние «сняты с рекламы сейчас», и оно не двигалось
при смене периода — читалось как поломка. Подвижным его не сделать (истории
статусов у Афины нет), зато сами снятия датированы, поэтому состояние
заменено событием: приток и отток за одно и то же окно. Считается второй
метрикой то же правило причины, что и в чипе снятий, иначе одно снятие
попадало бы в два места дашборда с разными числами.
...
Метрика «снято за период» проверяет и дату, и причину. Одной даты мало:
Афина проставляет `removed_from_ad_at` даже там, где причину не выбрали
вовсе, и не чистит её при возврате объекта в рекламу. Статус при этом не
проверяется, в отличие от плитки состояния: объект, снятый внутри периода и
потом возвращённый в рекламу, всё равно был снят, и в истории периода ему
место.

Окна периода здесь свои и короче общих для дашборда: сегодня, 7 дней, 30
дней, текущий квартал и произвольные даты. Раздел считает по снимку рабочих
карточек, и на длинных окнах метрика спрашивала бы о событиях, которых в
снимке уже нет — объект давно в архиве. Пресет из другого раздела приходит
по ссылке или правкой адреса и молча сводится к 30 дням: считать по окну,
которого нет среди чипов, значит показать числа неизвестно за что.

### Кто что видит

Администратор видит всё. РОП — только свой отдел, только чипы «Сейчас в
рекламе» и «Опубликованы на сайте», без среза «Все» и без журнала снятий:
первое Афина считает по всей базе одним числом `/summary`, второе отдаёт
журналом без фильтра по отделу, и сузить их до отдела нечем.

Связывает отделы карта в `AFINA_DEPARTMENT_MAP_JSON`, и составляется она
руками: справочники разные и по имени не сходятся — в Битриксе отдел зовётся
«Трофимова», в Афине «Отдел Трофимовой». Подобрать пару автоматически значило
бы угадывать падеж, а ошибка здесь показала бы РОПу чужой отдел. Отдел без
пары остаётся без раздела — конструкция закрывается при сбое.

Заготовку печатает `scripts/afina_departments.py`: он показывает обе стороны
и готовую строку для `.env`, в которой остаётся вписать названия.

Ограничение применяется к выборке, а не к ссылкам: `?department=` чужого
отдела, `?filter=all`, `?view=removals` и `?id=` чужого объекта проверяются
там, где происходит обращение к данным. Ссылок на недоступное на странице
нет вовсе, но до них можно добраться правкой адреса, и JSON `/api/objects`
запрашивают напрямую — поэтому проверка стоит в обоих обработчиках, а не в
одном.
