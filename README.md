...
Счётчики следуют за фильтром и поиском: без них это вся база одним запросом
к `/summary`, с ними — те же срезы внутри выборки. `/summary` фильтров не
принимает, поэтому суженные числа берутся из `total` выдачи, запрошенной
страницей в одну строку: четыре счётных запроса вместо одного, и только когда
выборка сужена.
