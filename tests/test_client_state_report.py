...
def test_a_card_in_both_sections_is_printed_once():
    ...
    # С 01.09 «работу не начинали» печатается строкой, а не разбором, —
    # поэтому считаем саму карточку, а не заголовок её разбора.
    assert body.count("#10 ЖК «Hide»") == 1
    ...
    # Карточка «работу не начинали» с 31.08 печатается в своём разделе, а
    # из тревоги уходит целиком: под двумя заголовками её больше нет.
    assert "РАБОТУ НЕ НАЧИНАЛИ — 1" in body
