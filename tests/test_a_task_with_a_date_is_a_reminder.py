"""Дело со сроком: сегодняшнее — напоминание, вчерашнее — недоработка.
...
До него дело со сроком в 18:00, прочитанное в 10 утра, не рассматривалось
...
Вторая половина того решения — «если дело просрочено, то надо поставить
флажок напомнить» — отменена 01.09: «считать что недоработка брокера, если
дело просрочено и результат не написан в комментарии в таймлайн». Отмена
идёт в паре с PROVEN_BY_OPEN_TASK: раз живое дело на контроле СНИМАЕТ
претензию к темпу, то дело, срок которого прошёл впустую, — это ровно тот
случай, где контроль оказался фикцией.

Граница между ними осталась та же и осталась календарной: день срока.
"""
...
# ── Место в лестнице ───────────────────────────────────────────────────
def test_the_calendar_day_divides_the_reminder_from_the_reproach():
    """Один день разницы — и разные разделы отчёта (01.09).

    Пара держится вместе: развести её можно только осознанно, потому что
    REMINDERS решает разом шапку строки, скобки с доводом и раздел.
    """
...
def test_both_codes_keep_their_own_argument():
    """Ни напоминание, ни претензия без даты дела РОПу не проверяемы."""
