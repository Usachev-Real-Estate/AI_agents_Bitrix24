...
def test_a_standing_task_turns_a_loss_into_a_shortfall():
    ...
    Без дела карточка идёт в тревогу и в свой список «работу не начинали»
    (с 31.08 он заменяет ей недоработку — повторять претензию нечем).
    Со стоящим делом тревоги нет, и остаётся ровно недоработка.
    """
    assert _sections(_card(GAP_NO_TRACE)) == "теряем"
    assert _sections(
