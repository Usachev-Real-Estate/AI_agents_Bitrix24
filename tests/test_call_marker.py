"""Работа описана комментарием, а разговора по карточке не видно.
...
def test_an_incoming_call_clears_the_mark_too():
    """Направление агентство решило не различать: важен факт разговора."""
    assert _mark([_comment(10.0), _call(10.0, direction=1)]) is False


def test_an_old_call_does_not_clear_the_mark():
...
def test_has_any_call_ignores_tasks_and_comments():
    assert has_any_call([_comment(1.0)]) is False
    assert has_any_call([{"kind": "activity", "type_id": 1, "direction": 2}]) is False
    assert has_any_call([_call(1.0, direction=2)]) is True
    assert has_any_call([_call(1.0, direction=1)]) is True


def test_the_mark_is_silent_inside_the_stage_grace():
    """#16976 стояла в «рано судить» — и тут же получала пометку."""
    events = [_comment(2.0)]
    assert comment_without_a_call(
        events, profile=BUYER_PROFILE, stage_id=STAGE,
