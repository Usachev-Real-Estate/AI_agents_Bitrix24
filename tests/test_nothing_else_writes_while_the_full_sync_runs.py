...
def test_the_client_layer_starts_after_the_full_sync_begins():
    """Клиентский слой читает витрину — значит не во время её пересборки.

    Слой берёт портфель из витрины как источник правды: сколько карточек,
    чьи они и что на них происходило. Витрина, прочитанная на середине
    полной сверки, отдаст половину — и слой честно и молча соберёт половину
    портфеля, потому что «мало карточек» он от «мало данных» отличить не
    может. Потери при этом не будет: ключи посчитаются заново следующей
    ночью. Будет день, когда список клиентов вдвое короче, и никто не знает
    почему.

    Писателем витрины слой не является (читает `readonly=True`), поэтому
    проверки выше его не касаются — блокировок он не берёт. Расписание ему
    нужно по другой причине, и держит её эта проверка.

    Запас тот же, что у писателей: час. Точной длительности полной сверки
    никто не мерил, и число это оценка — но оценка, записанная в тесте,
    переживает того, кто её делал, а записанная в голове нет.
    """
    full_at = _start(next(
        e for e in _by_script(ETL) if e["sched"].split()[1] != "*"
    )["sched"])
    layer = next(e for e in _entries() if e["script"] == "src/clients/build.py")

    assert _start(layer["sched"]) - full_at >= RUNWAY_MIN, (
        "пересборка клиентского слоя начинается слишком близко к полной "
        "сверке витрины — она прочитает недособранную витрину"
    )
