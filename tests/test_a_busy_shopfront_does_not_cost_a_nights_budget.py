...
class _Model:
    ...
    Задержка не для красоты. На боевом портале ответ идёт около трёх секунд,
    и в момент падения записи очередь пула полна. Мгновенная заглушка
    разбирала бы все двадцать карточек раньше, чем случится первая ошибка, —
    и тест на отмену очереди проходил бы, ничего не проверяя.

    Ждём через Event, а не time.sleep: его в этом файле подменяют.
    """

    def __init__(self, content='{"promised": "Позвонить в пятницу"}',
                 latency: float = 0.0):
        self.content, self.calls, self.latency = content, [], latency
...
def test_a_failed_write_stops_the_spending(mart, monkeypatch):
    ...
    Запись здесь роняется сразу, без повторов: повторы проверены выше, а тут
