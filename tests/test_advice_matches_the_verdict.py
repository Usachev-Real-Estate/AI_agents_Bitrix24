...
Отвечать претензии ожиданием нельзя — вот что здесь проверяется, и это
верно независимо от того, снимает дело претензию или нет.

Основание сменилось 01.09 (#13636): живое дело брокера теперь претензию к
темпу снимает, а решение 28.08 «дело это план, а не работа» отменено. Тесты
ниже от этого не устарели: норма этапа в них 1 дн., а дело стоит на третий —
за горизонтом, и претензия остаётся ровно тем случаем, ради которого горизонт
и заведён. Карточка, у которой дело укладывается в норму, до совета вообще не
доходит: она подтверждена и в отчёт не выводится.
"""
...
def test_a_standing_task_does_not_answer_an_unproven_card():
    """Дело ЗА НОРМОЙ этапа претензию не снимает — совет отвечает ей.
