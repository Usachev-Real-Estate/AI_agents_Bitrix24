...
def test_a_rop_does_not_see_the_companys_edge(agency, costs):
    """Рубеж — число собственника, и РОПу его видеть нечего.

    Порог считается из расходов ВСЕЙ компании, а факт на суженном
    соединении — только по отделам РОПа. Их разность не значит ничего:
    «не хватает 22 млн» при том, что компания в целую может быть у самого
    рубежа. Хуже того, из такой строки читается размер расходов агентства
    — число, которое собственник РОПу не называл.

    Совет, выросший отсюда, довершает картину: «Соберите РОПов и
    разберите» — распоряжение директора, и на экране РОПа оно выглядит
    так, будто это он собирает РОПов.

    Закрывается там же, где живёт область видимости, а не в правиле: тогда
    исчезает и с «Пульса», и из «Плана на день», и из совета сразу.
    """
    with scoped_session(Scope.departments([DEPT])) as conn:
        theirs = pulse_module.pulse(conn, Q3, today=MID)

    assert theirs["breakeven"] is None
    # Свои числа при этом на месте: закрыт рубеж, а не весь экран.
    assert theirs["fact"] > 0


def test_the_owner_still_sees_it(agency, costs):
    """Обратная сторона: для администратора рубеж обязан остаться."""
    assert _pulse()["breakeven"] is not None
