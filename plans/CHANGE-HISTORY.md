...
8. **Считать любой комментарий на сделке** — заменено на broker+ROP only (step-59).
...
- **Пайплайн:** collectors → analysts (lead LLM+rule1; buyer/calls deterministic) → merge → dispatcher → ROP chats + summary chat.
- **LLM:** только lead spam / нецелевой (`LEAD_ANALYST_PROMPT`).
- **Buyer «Показ» (rule 3):** пока сложная логика (дела + дата показа / plan comment) — **не** упрощённый вариант step-62.
