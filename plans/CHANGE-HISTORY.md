...
Правила QC «состояние клиента» (температура, оценка карточки, доказательства работы, разделы отчёта) вынесены отдельно: [`qc-client-state-rules.md`](qc-client-state-rules.md) — сверяйся с ним перед любой правкой этого модуля.

Разбор мест, где правила проекта спорят друг с другом (основной аудит против QC, три часа «рано судить», решения агентства без реализации): [`rule-contradictions-audit.md`](rule-contradictions-audit.md).
